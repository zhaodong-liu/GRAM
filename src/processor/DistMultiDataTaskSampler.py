...
class DistMultiDataTaskSampler(SingleMultiDataTaskSampler):
    ...
    def __init__(self, dataset, batch_size, num_replicas, rank, seed, shuffle=True):
        ...
        for ds in self.dataset.datasets:
            for task in ds.task_data:
                self.dataset_task_size.append(
                    math.ceil(len(ds.task_data[task]) / self.num_replicas)
                )
    ...
    def __len__(self):
        return (
            self.batch_size
            * math.ceil(self.largest_task_size / self.batch_size)
            * len(self.dataset_task_size)
        )
